...
struct MapThumbnailWithExpand: View {
    ...
    @State private var annotationItemsChat = [MyAnnotationItem]()
    ...
    var body: some View {
        VStack {
            Button(action: { showingFullMap.toggle() }) {
                Map(coordinateRegion: $region, annotationItems: annotationItemsChat) { item in
            ...
            NavigationLink(destination: MapView(location: position, annotationItemsChat: annotationItemsChat), isActive: $showingFullMap) {
                EmptyView()
            }
        }
    ...
    func setupLocation() {
        ...
        annotationItemsChat.append(MyAnnotationItem(coordinate: position))
    }
}
